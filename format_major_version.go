...
const (
	...
	FormatRangeKeys
	// FormatMinTableFormatPebblev1 is a format major version guarantees that new
	...
	// FormatPreBlockPropertiesMarked is a format major version that guarantees
	// that all sstables with a table format version pre-Pebblev1 (i.e. those that
	// are guaranteed to not contain block properties) are marked for compaction
	// in the manifest. Ratcheting to FormatPreBlockPropertiesMarked will block
	// (without holding mutexes) until the scan of the LSM is complete and the
	// manifest has been rotated.
	FormatPreBlockPropertiesMarked

	// 23.1 versions.

	// FormatTablePropertiesMarkedCompacted is a format major version that
	...
	FormatTablePropertiesMarkedCompacted
	...
	// FormatMajorVersion.
	FormatNewest FormatMajorVersion = FormatTablePropertiesMarkedCompacted
)
...
func (v FormatMajorVersion) MaxTableFormat() sstable.TableFormat {
	...
	case FormatRangeKeys, FormatMinTableFormatPebblev1, FormatPreBlockPropertiesMarked, FormatTablePropertiesMarkedCompacted:
...
func (v FormatMajorVersion) MinTableFormat() sstable.TableFormat {
	...
	case FormatDefault, FormatMostCompatible, formatVersionedManifestMarker,
		...
		FormatSplitUserKeysMarked, FormatSplitUserKeysMarkedCompacted, FormatRangeKeys:
		return sstable.TableFormatLevelDB
	case FormatMinTableFormatPebblev1, FormatPreBlockPropertiesMarked, FormatTablePropertiesMarkedCompacted:
		return sstable.TableFormatPebblev1
	default:
		panic(fmt.Sprintf("pebble: unsupported format major version: %s", v))
	}
...
var formatMajorVersionMigrations = map[FormatMajorVersion]func(*DB) error{
	...
	FormatSplitUserKeysMarked: func(d *DB) error {
		...
		if err := d.markFilesLocked(markFilesWithSplitUserKeys(d.opts.Comparer)); err != nil {
			return err
		}
	...
	FormatPreBlockPropertiesMarked: func(d *DB) error {
		...
		if err := d.markFilesLocked(markFilesPreBlockProps(d.tableCache)); err != nil {
			return err
		}
		return d.finalizeFormatVersUpgrade(FormatPreBlockPropertiesMarked)
	},
	FormatTablePropertiesMarkedCompacted: func(d *DB) error {
		// Before finalizing the format major version, rewrite any sstables
		// still marked for compaction. Note all format major versions
		// migrations are invoked with DB.mu locked.
		if err := d.compactMarkedFilesLocked(); err != nil {
		...
		return d.finalizeFormatVersUpgrade(FormatTablePropertiesMarkedCompacted)
	},
}
...
var markFilesWithSplitUserKeys = func(cmp *Comparer) findFilesFunc {
	return func(v *version) (found bool, files [numLevels][]*fileMetadata, _ error) {
		...
		for l := numLevels - 1; l > 0; l-- {
			...
			for f := iter.First(); f != nil; f = iter.Next() {
				if prevUserKey != nil && cmp.Equal(prevUserKey, f.Smallest.UserKey) {
...
// markFilesPreBlockProps scans the LSM for files that do not support block
// properties (i.e. a table format version pre-Pebblev1).
var markFilesPreBlockProps = func(tc *tableCacheContainer) findFilesFunc {
